...
def main():
    ...
    parser.add_argument("--save_interval",type=int,default=1,help="每隔多少个 epoch 保存一次检查点（默认1，即每个epoch都保存）")
    ...
    data_dir = project_root / args.data_dir  # 基于项目根目录解析 data_dir
    ...
    model = PGCoverageSeq2Seq( # 使用带 Coverage 的模型
        ...
        pad_idx=pad_idx,
        cov_loss_weight=args.coverage_loss_weight  # 关键补充：传递命令行中的覆盖损失权重
    ).to(device)
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        tb_writer.add_scalar("Train/TotalLoss", train_total_loss, epoch) #tensorboard visual
        ...
        if epoch % args.save_interval == 0:  # 仅当 epoch 是间隔的倍数时保存
            ckpt_path = Path(args.save_dir) / f"checkpoint_epoch_{epoch}.pt"  # 统一命名格式
            torch.save({
                "epoch": epoch,
                "model_state_dict": model.state_dict(),
                "optimizer_state_dict": optimizer.state_dict(),
            ...
            logger.info(f"保存检查点（间隔 {args.save_interval} 个 epoch）: {ckpt_path}")
