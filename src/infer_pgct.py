...
def main(args):
    ...
    output_file = args.output or os.path.join(eval_cfg.get('output_dir', '../outputs_pgct'), eval_cfg.get('output_file', 'test_summaries.json'))

    vocab = Vocab.load(vocab_path)
    print(f"✅ 加载词表：{vocab_path}（大小：{len(vocab)}）")

    model, model_cfg = load_pgct_model(checkpoint_path, len(vocab), vocab.pad_idx, device, config)
