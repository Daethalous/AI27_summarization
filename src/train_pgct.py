...
# -------------------------------------------------------------------------
# 关键改动 1: 引入 PGCTModel 和解码函数
from models.pgct.pgct_model import PGCTModel 
from models.pgct.pgct_decoding import pgct_greedy_decode  # 用于生成验证集摘要
# -------------------------------------------------------------------------
...
def main():
    ...
    args.embed_size = args.embed_size or model_config.get("embed_size", 256)
    args.hidden_size = args.hidden_size or model_config.get("hidden_size", 256)  # 优先读取配置文件的hidden_size
...
if __name__ == "__main__":
    # 示例运行命令 (在 src 目录下): 
    # python train_pgct.py --config ../configs/pgct.yaml --batch_size 16
    main()
