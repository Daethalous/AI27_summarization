...
class PGCoverageDecoder(nn.Module):
    ...
    def __init__(
        ...
        dropout: float = 0.1,
        pad_idx: int = 0,
        cov_loss_weight: float = 1.0  # 新增：覆盖损失权重（默认1.0，与论文一致）
    ) -> None:
        super().__init__()
        self.vocab_size = vocab_size
        self.hidden_size = hidden_size
        self.pad_idx = pad_idx
        self.cov_loss_weight = cov_loss_weight  # 保存为类属性，供forward使用
    ...
    def forward(
        ...
        # 关键修改：用类属性 self.cov_loss_weight 替代未定义的 COV_LOSS_WEIGHT
        coverage_loss = self.cov_loss_weight * all_coverage_loss_t.mean()
